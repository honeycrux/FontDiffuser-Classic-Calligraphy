...
def load_test_profile(profile_dir: str):
    ...
    if len(profile_info_map) != len(profile_info):
        raise ValueError("Erorr when loading a test profile: test indices are not continuous starting from 0")
...
def main():
    ...
    # Results location
    results_output_dir = 'outputs/eval_naive_few_shot'
    ...
    for test_idx, test_info in enumerate(profile_info):
        ...
        for file_idx, file_info in enumerate(test_info['test_info'].values()):
            print(f"[{test_idx + 1}/{total_tests}][{file_idx + 1}/{total_files}] ", end="")
            ...
            if out_image.size != character_image.size:
                out_image = out_image.resize(character_image.size, Image.Resampling.BILINEAR)
        ...
        print()
        print(f"[Eval] Sampling and evaluation for test {test_idx} ended, costing time {round_time}s")
    ...
    fid_values = torch.tensor([test_result['fid'] for test_result in test_results.values()])
    ssim_values = torch.tensor([test_result['ssim'] for test_result in test_results.values()])
    lpips_values = torch.tensor([test_result['lpips'] for test_result in test_results.values()])
    l1_values = torch.tensor([test_result['l1'] for test_result in test_results.values()])
