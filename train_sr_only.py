...
def main():
    ...
    def compute_loss(samples):
        ...
        content_content_feture, content_content_residual_features = content_encoder(
            content_images
        )
        ...
        for style_batch_item in style_batch:
            style_content_feature, style_content_residual_features = content_encoder(
                style_batch_item
            )
        ...
        style_reconstruction_loss = F.mse_loss(
            style_feature_pred.float(), style_feature_real.float(), reduction="mean"
        )
    ...
    for epoch in range(num_train_epochs):
        ...
        for step, samples in enumerate(train_dataloader):
            ...
            if is_checkpoint_step and is_on_main_process:
                ...
                torch.save(
                    get_model(model).state_dict(), f"{save_dir}/style_reconstructor.pth"
                )
                torch.save(
                    {
