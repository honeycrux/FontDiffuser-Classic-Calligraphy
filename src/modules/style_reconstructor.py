...
class StyleReconstructor(nn.Module):
    def __init__(self, k_shot, max_k=256):
        super().__init__()
    ...
    def forward(
        ...
        if style_style_feature.shape[1] < self.max_k:
            style_shape = (
                style_style_feature.shape[0],
                self.max_k - style_style_feature.shape[1],
                *style_style_feature.shape[2:],
            )
            style_style_feature = torch.cat(
                [
                    style_style_feature,
                    torch.zeros(style_shape).to(style_style_feature.device),
                ],
                dim=1,
            )
        ...
        unpacked = st2_output.reshape(
            B, K, H * W, C
        )  # shape it into 3D for channel attention (involving conv2d)
        ca1_output = self.ca1(
            inputs=unpacked,
        )
        # print("ca1_output", ca1_output.shape)

        final_style = ca1_output.reshape(B, H, W, C).permute(
            0, 3, 1, 2
        )  # back to style feature shape
