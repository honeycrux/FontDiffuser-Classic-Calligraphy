...
def arg_parse():
    ...
    parser.add_argument("--ttf_path", type=str, default=None)      # The ttf path
...
def image_process_with_path(args) -> Union[None, tuple[Image.Image, list[Image.Image]]]:
    ...
    if args.character_input:
        assert isinstance(content_character, str), "The content_character should be str."
        if not (args.ttf_path and is_char_in_font(font_path=args.ttf_path, char=content_character)):
            return None
        font = load_ttf(ttf_path=args.ttf_path)
        content_image = ttf2im(font=font, char=content_character)
    else:
        assert isinstance(content_image_path, str), "The content_image_path should be str."
        content_image = Image.open(content_image_path).convert('RGB')

    assert isinstance(style_image_path, str), "The style_image_path should be str."
    style_images_dir = Path(style_image_path)      # The style image directory
    ...
    assert all([isinstance(style_image, Image.Image) for style_image in style_images]), "The style images should be PIL.Image.Image."

    return content_image, style_images

def image_process_with_image(args, content_image, style_images) -> Union[None, tuple[Image.Image, list[Image.Image]]]:
    content_character = args.content_character
...
def image_process(args, content_image=None, style_images=None) -> Union[None, tuple[torch.Tensor, torch.Tensor, Image.Image, list[Image.Image]]]:
    ## Get PIL images
    ...
    style_images = [style_transforms(style_image)[None, :] for style_image in style_images_pil]
...
def sampling(args, pipe, content_image=None, style_images=None):
    ...
    image_process_output = image_process(args=args, content_image=content_image, style_images=style_images)
